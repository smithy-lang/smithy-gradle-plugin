...
public class CustomTraitTest {
    ...
    public void testCustomTrait() {
        Utils.withCopy("jar-plugin/custom-trait", buildDir -> {
            ...
            assertSame(requireNonNull(result.task(":custom-string-trait:smithyBuild")).getOutcome(),
                    TaskOutcome.SUCCESS);
            assertSame(requireNonNull(result.task(":custom-string-trait:smithyJarValidate")).getOutcome(),
                    TaskOutcome.SUCCESS);
