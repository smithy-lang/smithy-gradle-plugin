...
public abstract class SmithyJarStagingTask extends DefaultTask {
    ...
    Provider<Path> getSourcesPluginPath() {
        return getInputDirectory().getAsFile()
                .zip(getProjection(),
                        (input, projection) -> SmithyUtils
                                .getProjectionPluginPath(input, projection, SOURCES_PLUGIN_NAME));
    }

    @Internal
    Provider<File> getSmithyResourceTempDir() {
        return getOutputDir().getAsFile().map(file -> SmithyUtils.getSmithyResourceTempDir(getName(), file));
    }
